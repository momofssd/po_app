...
export const ResultsTable: React.FC<ResultsTableProps> = ({ data, onUpdate }) => {
  if (data.length === 0) {
    return (
    ...
    <div className="w-full bg-white shadow-xl shadow-slate-200/50 rounded-2xl overflow-hidden border border-slate-100 flex flex-col h-full">
      <div className="px-8 py-5 border-b border-slate-100 bg-white flex justify-between items-center sticky top-0 z-20">
        <div className="flex items-center space-x-3">
          ...
          <h3 className="text-lg font-bold text-slate-800">Extracted Line Items <span className="ml-2 text-xs py-1 px-2.5 bg-indigo-50 text-indigo-600 rounded-full">{data.length}</span></h3>
        </div>
        <button
          onClick={() => {
          ...
          <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
            <tr>
              {[
                ...
                <th key={header.label} scope="col" className={`px-4 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider ${header.width}`}>
                  {header.label}
                </th>
              ))}
            </tr>
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {data.map((row, index) => (
              <tr key={index} className="hover:bg-indigo-50/30 transition-colors group">
                <td className="px-2 py-4 text-[10px] text-slate-400 font-mono whitespace-normal break-all leading-tight border-l-2 border-transparent group-hover:border-indigo-500" title={row.sourceFile}>
                  {row.sourceFile || '-'}
                </td>
                ...
                <td className="px-2 py-3">
                  <input
                    ...
                    onChange={(e) => onUpdate(index, 'purchaseOrderNumber', e.target.value)}
                ...
                <td className="px-2 py-3">
                  <textarea
                    ...
                    onChange={(e) => onUpdate(index, 'customerName', e.target.value)}
                ...
                <td className="px-2 py-3">
                  <input
                    ...
                    onChange={(e) => onUpdate(index, 'materialNumber', e.target.value)}
                ...
                <td className="px-2 py-3">
                  <input
                    ...
                    onChange={(e) => onUpdate(index, 'orderQuantity', e.target.value)}
                ...
                <td className="px-2 py-3">
                  <input
                    ...
                    onChange={(e) => onUpdate(index, 'unitOfMeasure', e.target.value)}
                ...
                <td className="px-2 py-3">
                  <input
                    ...
                    onChange={(e) => onUpdate(index, 'requiredDeliveryDate', e.target.value)}
